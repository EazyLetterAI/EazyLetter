...
export default function GenerateLetter() {
  ...
  const letter = <CoverLetter letterContents={letterEditorValue} name={nameEditorValue} email={emailEditorValue} phone={phoneEditorValue} />;
  ...
  return (
    <div className="h-screen">
      ...
      <div className="z-10 flex h-3/5 w-screen flex-col rounded-lg border border-slate-400 bg-gradient-to-r from-slate-50 p-2 px-8 shadow-xl sm:sticky sm:top-1/4 sm:float-end sm:w-5/12 sm:px-2">
        <PDFViewer width={"100%"} className="flex-grow" showToolbar={false}>
          {letter}
        </PDFViewer>
        <PDFDownloadLink document={letter}>
          <button
